...
class Grid {
  ...
  pickPaperRoll([x, y]: Position): void {
    this.values[y][x] = ".";
  }
}
...
(async function () {
  ...
  let pickableRolls: Position[];
  let totalPicked = 0;

  do {
    pickableRolls = grid.pickablePaperRolls;

    pickableRolls.forEach((roll) => {
      grid.pickPaperRoll(roll);
    });

    totalPicked += pickableRolls.length;
  } while (pickableRolls.length > 0);
