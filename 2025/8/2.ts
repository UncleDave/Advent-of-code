...
class JunctionBoxes {
  ...
  public lastConnectedBoxPair?: JunctionBoxPair;
  ...
  getClosestBoxes(): JunctionBoxPair[] {
  ...
  createCircuits(): void {
    const closestBoxes = this.getClosestBoxes();
    ...
    for (const pair of closestBoxes) {
      ...
      this.lastConnectedBoxPair = pair;
    }
  }
}
...
(async function () {
  ...
  junctionBoxes.createCircuits();

  console.log(
    junctionBoxes.lastConnectedBoxPair?.boxes.reduce(
      mapReducePower((x) => x.position[0]),
      1,
    ),
  );
})();
