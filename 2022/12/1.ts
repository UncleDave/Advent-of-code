...
class Graph {
  ...
  constructor(input: string[]) {
    ...
    this.nodes.forEach(node => {
      node.distanceFromStartNode = node.distance(this.startNode);
  ...
  routeDistance(): number {
    const open = new NodePriorityQueue(this.startNode);
    const closed: Node[] = [];

    while (open.peek() !== this.endNode) {
      const node = open.pop()!;
      closed.push(node);
...
(async function () {
  ...
  console.log(graph.routeDistance());
})();
